...
- Il proprio nome PG viene evidenziato quando compare nelle battute altrui, come fa lot nativamente in chat — così si nota subito chi ci ha citato. Il nome viene letto una sola volta all'avvio da `b_all.asp` (stessa pagina della toolbar principale); se non riesce a determinarlo l'evidenziazione resta semplicemente disattivata, senza impatto sul resto. Il nome di un altro PG noto in questa sessione viene evidenziato allo stesso modo ma più leggero (stesso colore, senza grassetto/sottolineatura) — per notare le citazioni altrui senza confonderle con la propria
...
- Token piazzati dal Fato sulla mappa-quest di lot (segnalini che non passano mai da un messaggio in chat, quindi altrimenti invisibili a questo strumento): mostrati con lo stesso stile esatto del client reale (cerchio colorato + anello al posto dello stemma, nome in corsivo) a zoom basso, sagoma fantasma a zoom modellino. Letti solo in live, solo nei tre momenti in cui si è sull'ultimo messaggio (si atterra lì con un click, si preme "torna al live", o arriva un nuovo messaggio mentre già lì) — mai un polling continuo. Ogni lettura riuscita resta in uno storico in memoria (perso al reload): scorrendo indietro tra i messaggi si ricostruisce come appariva il board del Fato in quel momento, con la granularità dei soli tre trigger, non un log continuo — prima della primissima lettura, nessun segnalino
- Stessa lettura usata anche come secondo segnale di uscita per i PG (in aggiunta alla convenzione `//`, non al suo posto): se un PG che stiamo già disegnando da chat non compare più tra le posizioni note a quello strumento, stessa dissolvenza-poi-rimozione al turno successivo. La *posizione* di un PG resta sempre e solo quella scritta in chat — questo segnale serve solo a intercettare un'uscita che la convenzione `//` da sola potrebbe non cogliere
...
- Personaggi come stemma compatto (zoom basso) o modellino intero a layer corpo/vestito (zoom alto), con ventaglio automatico quando più PG condividono una cella, PG che sta parlando evidenziato (cella colorata, e sul modellino intero anche freccia + glow). Se nel Baule dei Ricordi di Alderick c'è una voce con un modellino intero custom per quel PG (`<summary>model</summary>` per Alderick stesso, `<summary>model-nome</summary>` per gli altri), quello sostituisce lo stack di layer standard — solo se le sue proporzioni sono compatibili con la sagoma standard, altrimenti fallback automatico al modellino a layer
- Se non c'è una scheda da cui ricostruire il modellino (parlante non riconosciuto, es. un drago in mutaforma) ma il messaggio porta comunque una coordinata e un'icona razza, il token compare comunque sulla mappa: icona razza reale a zoom basso, sagoma "fantasma" sfocata e pulsante a zoom alto (stessa per ogni caso, draghi compresi) — stesso glow sul PG attivo e stessa ombra ai piedi di un modellino vero
