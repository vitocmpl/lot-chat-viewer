...
- Personaggi come stemma compatto (zoom basso) o modellino intero a layer corpo/vestito (zoom alto), con ventaglio automatico quando più PG condividono una cella, PG che sta parlando evidenziato (cella colorata, e sul modellino intero anche freccia + glow). Se il giocatore pubblica un modellino intero custom nel Baule dei Ricordi (voce marcata `<summary>model</summary>`), quello sostituisce lo stack di layer standard — solo se le sue proporzioni sono compatibili con la sagoma standard, altrimenti fallback automatico al modellino a layer
